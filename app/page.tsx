...
export default function Home() {
  ...
  return (
    <main className="mira-site market-v8" dir={language === "AR" ? "rtl" : "ltr"}>
      <header className="mira-header">
        <a className="mira-brand" href="#anasayfa" aria-label="RealYerin ana sayfa"><span className="mira-monogram">R</span><span className="mira-wordmark">REALYERİN<small>TÜRKİYE'NİN EMLAK PLATFORMU</small></span></a>
      ...
      <section className="mira-hero" id="anasayfa">
        ...
        <div className="hero-frame" aria-hidden="true"><span>REALYERİN / 2026</span><span>EMLAKTA YENİ NESİL</span><span>GÜVENLİ İLAN</span></div>
        <div className="hero-copy"><p className="hero-kicker"><i /> HAFTANIN ÖNERİLEN İLANI / 0{heroIndex + 1}</p><span className="hero-location">{heroListing.district}, {heroListing.city}</span><h1>{heroListing.title}</h1><div className="hero-details"><span><b>{heroListing.rooms}</b><small>ODA</small></span><span><b>{heroListing.area}</b><small>BRÜT ALAN</small></span><span><b>{heroListing.floor}</b><small>KONUM</small></span></div><div className="hero-links"><button type="button" onClick={() => setSelected(heroListing)}>İlanı incele<span>↗</span></button><a href="#portfoy">Tüm ilanlar</a></div></div>
        <aside className="hero-price"><span>İlan fiyatı</span><strong>{heroListing.price}</strong><small><i /> RealYerin doğrulanmış ilan</small></aside>
        ...
        <form className="mira-search" onSubmit={submitSearch}><div className="mira-search-tabs" role="group" aria-label="İlan türü">{(["Satılık", "Kiralık"] as const).map((item) => <button type="button" className={intent === item ? "active" : ""} onClick={() => setIntent(item)} key={item}>{item}</button>)}</div><label><span>Nerede?</span><select value={city} onChange={(event) => setCity(event.target.value)}><option value="Tümü">Tüm Türkiye</option><option>İstanbul</option><option>İzmir</option><option>Ankara</option><option>Bursa</option></select></label><label><span>Gayrimenkul tipi</span><select value={kind} onChange={(event) => setKind(event.target.value)}><option>Tümü</option><option>Konut</option><option>Villa</option><option>Arsa</option><option>İş yeri</option></select></label><button className="mira-search-button" type="submit"><span>{filteredListings.length} güncel ilan</span><b>İlanları göster →</b></button></form>
      </section>

      <section className="ry-trust" aria-label="RealYerin platform bilgileri">
        <p><i /> Türkiye'nin yalnızca emlağa odaklanan yeni nesil ilan platformu</p>
        <div><span><strong>24.800+</strong><small>Güncel ilan</small></span><span><strong>81</strong><small>Şehirde erişim</small></span><span><strong>7/24</strong><small>İlan kontrolü</small></span><span><strong>12 ay</strong><small>Ücretsiz üyelik</small></span></div>
      </section>
      ...
      <section className="ry-categories" aria-label="Emlak kategorileri">
        ...
        <div className="ry-category-grid">
          {[["⌂", "Konut", "18.240 ilan"], ["⌁", "Villa", "3.180 ilan"], ["◇", "Arsa", "2.760 ilan"], ["▦", "İş yeri", "1.940 ilan"]].map(([icon, title, count]) => <button type="button" key={title} onClick={() => { setKind(title); document.getElementById("portfoy")?.scrollIntoView({ behavior: "smooth" }); }}><b>{icon}</b><span>{title}</span><small>{count}</small><i>→</i></button>)}
        </div>
      </section>
